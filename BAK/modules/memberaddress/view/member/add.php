<?php if ($data['page']['memberaddress_add']==2) { ?>
<div class="error">Member Address could not be created. Please try again.</div>
<?php } ?>
<form name="add_form" class="admin_table_nocell"  id="add_form" action="<?php echo $data['config']['SITE_DIR']; ?>/admin/memberaddress/addprocess" method="post">
  <table border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td><select name="MemberID" class="chosen_simple">
            ...
            <option value="<?php echo $data['content_param']['member_list'][$i]['ID']; ?>"><?php echo $data['content_param']['member_list'][$i]['FirstName']; ?></option>
            <?php } ?>
          </select></td>
    </tr>
    <tr>
    ...
    <tr>
      <th scope="row">&nbsp;</th>
      <td><input type="submit" name="submit" value="Add" class="button" /> <a href="<?php echo $data['config']['SITE_DIR']; ?>/admin/membermemberaddress/index"><input type="button" value="Cancel" class="button" /></a></td>
    </tr>
  </table>
</form>
