<?php if ($data['page']['dealeraddress_add']==2) { ?>
<div class="error">Dealer Address could not be created. Please try again.</div>
<?php } ?>
<form name="add_form" class="admin_table_nocell"  id="add_form" action="<?php echo $data['config']['SITE_URL']; ?>/admin/dealeraddress/addprocess" method="post">
  <table border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td><select name="DealerID" class="chosen_simple">
            ...
            <option value="<?php echo $data['content_param']['dealer_list'][$i]['ID']; ?>"><?php echo $data['content_param']['dealer_list'][$i]['FirstName']; ?></option>
            <?php } ?>
          </select></td>
    </tr>
    <tr>
    ...
    <tr>
      <th scope="row">&nbsp;</th>
      <td><input type="submit" name="submit" value="Add" class="button" /> <a href="<?php echo $data['config']['SITE_URL']; ?>/admin/dealerdealeraddress/index"><input type="button" value="Cancel" class="button" /></a></td>
    </tr>
  </table>
</form>
