<?php if ($data['page']['adtype_add']['error']['count']>0) { ?>
<?php } else { ?>
    <?php if ($data['page']['adtype_add']['ok']==1) { ?>
    <div class="notify">Ad Type created successfully.</div>
    <?php } ?>
<?php } ?>
<?php if ($data['page']['adtype_edit']['error']['count']>0) { ?>
<?php } else { ?>
    <?php if ($data['page']['adtype_edit']['ok']==1) { ?>
    <div class="notify">Ad Type edited successfully.</div>
    <?php } ?>
<?php } ?>
<form name="edit_form" class="admin_table_nocell"  id="edit_form" action="<?php echo $data['config']['SITE_URL']; ?>/admin/adtype/editprocess/<?php echo $data['content'][0]['ID']; ?>" method="post">
  <table border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      ...
      <td><input type="submit" name="submit" value="Update" class="button" />
        <a href="<?php echo $data['config']['SITE_URL']; ?>/admin/adtype/index">
