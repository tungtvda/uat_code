<?php if ($data['page']['merchant_add']['error']['count']>0) { ?>
    <div class="error">
        <ul>
            <?php if ($data['page']['merchant_add']['error']['Username']==1) { ?><li>The username "<?php echo $data['form_param']['Username']; ?>" is taken. Please try again with another username.</li><?php } ?>
            <?php if ($data['page']['merchant_add']['error']['NRIC']==1) { ?><li>The NRIC number <?php echo $data['form_param']['NRIC']; ?> exists in the database. Each merchant can only register once. Please try again with another NRIC number.</li><?php } ?>
            <?php if ($data['page']['merchant_add']['error']['Passport']==1) { ?><li>The Passport number <?php echo $data['form_param']['Passport']; ?> exists in the database. Each merchant can only register once. Please try again with another Passport number.</li><?php } ?>
        </ul>
    </div>
<?php } ?>
<form name="add_form" class="admin_table_noMobile" id="add_form" action="<?php echo $data['config']['SITE_URL']; ?>/admin/merchant/addprocess" method="post">
  <table border="0" Mobilespacing="0" Mobilepadding="0">
    ...
    <tr>
      ...
      <td><select name="DealerID" class="chosen">
            ...
            <option value="<?php echo $data['content_param']['dealer_list'][$i]['ID']; ?>" <?php if ($data['content_param']['dealer_list'][$i]['ID']==$_SESSION['merchant_AdminIndex']['param']['DealerID']) { ?> selected="selected"<?php } ?>><?php //echo $data['content_param']['dealer_list'][$i]['ID']; ?><?php echo $data['content_param']['dealer_list'][$i]['Name']; ?></option>
            <?php } ?>
          </select></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><select name="TypeID" class="chosen_simple">
          ...
          <option value="<?php echo $data['content_param']['merchanttype_list'][$i]['ID']; ?>" <?php if ($data['content_param']['merchanttype_list'][$i]['ID']==$data['form_param']['TypeID']) { ?>selected="selected"<?php } ?>><?php echo $data['content_param']['merchanttype_list'][$i]['Label']; ?></option>
          <?php } ?>
        </select></td>
    </tr>
    <tr>
      <th scope="row"><label>Date of Birth<span class="label_required">*</span></label></th>
      <td><input name="DOB" class="validate[required,custom[dmyDate]] datepicker mask_date" type="text" value="<?php echo $data['form_param']['DOB']; ?>" size="10" maxlength="10" /><span class="label_hint">(dd-mm-yyyy)</span></td>
    </tr>
    <tr>
      <th scope="row"><label>Expiry<span class="label_required">*</span></label></th>
      <td><input name="Expiry" class="validate[required,custom[dmyDate]] datepicker mask_date" type="text" value="<?php echo $data['form_param']['Expiry']; ?>" size="10" maxlength="10" /><span class="label_hint">(dd-mm-yyyy)</span></td>
    </tr>
    <tr>
