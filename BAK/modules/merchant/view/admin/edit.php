<?php if ($data['page']['merchant_add']['ok']==1) { ?>
<div class="notify">Merchant created successfully.</div>
<?php } ?>
<?php if ($data['page']['merchant_edit']['error']['count']>0) { ?>
    <div class="error">
        <ul>
            <?php if ($data['page']['merchant_edit']['error']['Username']==1) { ?><li>The username "<?php echo $data['form_param']['Username']; ?>" is taken. Please try again with another username.</li><?php } ?>
            <?php if ($data['page']['merchant_edit']['error']['NRIC']==1) { ?><li>The NRIC number <?php echo $data['form_param']['NRIC']; ?> exists in the database. Each merchant can only register once. Please try again with another NRIC number.</li><?php } ?>
            <?php if ($data['page']['merchant_edit']['error']['Passport']==1) { ?><li>The Passport number <?php echo $data['form_param']['Passport']; ?> exists in the database. Each merchant can only register once. Please try again with another Passport number.</li><?php } ?>
        </ul>
    </div>
<?php } else { ?>
    <?php if ($data['page']['merchant_edit']['ok']==1) { ?>
    <div class="notify">Merchant edited successfully.</div>
    <?php } ?>
<?php } ?>
<form name="edit_form" class="admin_table_noMobile"  id="edit_form" action="<?php echo $data['config']['SITE_URL']; ?>/admin/merchant/editprocess/<?php echo $data['content'][0]['ID']; ?>" method="post">
  <table border="0" Mobilespacing="0" Mobilepadding="0">
    ...
    <tr>
      ...
      <td><select name="DealerID" class="chosen">
          <?php for ($i=0; $i<$data['content_param']['dealer_list']['count']; $i++) { ?>
          <option value="<?php echo $data['content_param']['dealer_list'][$i]['ID']; ?>" <?php if ($data['content_param']['dealer_list'][$i]['ID']==$data['content'][0]['DealerID']) { ?> selected="selected"<?php } ?>><?php //echo $data['content_param']['dealer_list'][$i]['ID']; ?><?php echo $data['content_param']['dealer_list'][$i]['Name']; ?></option>
          <?php } ?>
        </select></td>
    </tr>
    <tr>
    ...
    <tr>
      <th scope="row"><label>Profile Type<span class="label_required">*</span></label></th>
      <td><select name="TypeID" class="chosen_simple">
          <?php for ($i=0; $i<$data['content_param']['merchanttype_list']['count']; $i++) { ?>
          <option value="<?php echo $data['content_param']['merchanttype_list'][$i]['ID']; ?>" 
              <?php if ($data['form_param']['TypeID']!="") { ?>
              <?php if ($data['content_param']['merchanttype_list'][$i]['ID']==$data['form_param']['TypeID']) { ?>selected="selected"<?php } ?>
              <?php } else { ?>              
              <?php if ($data['content_param']['merchanttype_list'][$i]['ID']==$data['content'][0]['TypeID']) { ?>selected="selected"<?php } ?>
              <?php } ?>><?php echo $data['content_param']['merchanttype_list'][$i]['Label']; ?></option>
          <?php } ?>
        </select></td>
    </tr>
    <tr>
    ...
    <tr>
      <th scope="row"><label>Expiry<span class="label_required">*</span></label></th>
      <td><input name="Expiry" type="text" class="validate[required,custom[dmyDate]] datepicker mask_date" value="<?php echo (($data['form_param']['Expiry']!="") ? $data['form_param']['Expiry'] : $data['content'][0]['Expiry']); ?>" size="10" maxlength="10" /><span class="label_hint">(dd-mm-yyyy)</span></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><input type="submit" name="submit" value="Update" class="button" />
        <a href="<?php echo $data['config']['SITE_URL']; ?>/admin/merchant/index">
