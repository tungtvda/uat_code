<?php if ($_SESSION['merchant']['Prompt']==1) { ?>
<div class="notify">To protect your account, please enter a new password.</div>
<?php } ?>
<?php if ($data['page']['merchant_password']['error']['count']>0) { ?>
    <?php if ($data['page']['merchant_password']['error']['Password']==1) { ?>
    <div class="error">Current password entered is incorrect. Please try again.</div>
    <?php } ?>
<?php } ?>
<form class="admin_table_nocell" name="password_form" id="password_form" method="post" action="<?php echo $data['config']['SITE_URL']; ?>/merchant/merchant/passwordprocess">
