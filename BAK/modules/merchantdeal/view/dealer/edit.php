...
<?php } else { ?>
    ...
    <div class="notify">Dealer Deal created successfully.</div>
    <?php } ?>
<?php } ?>
...
<?php } else { ?>
    ...
    <div class="notify">Dealer Deal edited successfully.</div>
    <?php } ?>
<?php } ?>
<form action="<?php echo $data['config']['SITE_URL']; ?>/dealer/merchantdeal/editprocess/<?php echo $data['content'][0]['ID']; ?>" method="post" enctype="multipart/form-data" name="edit_form" class="admin_table_nocell"  id="edit_form">
  <table border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      ...
      <td><select name="MerchantID" class="chosen">
          ...
          <option value="<?php echo $data['content_param']['merchant_list'][$i]['ID']; ?>" <?php if($data['content_param']['merchant_list'][$i]['Limit']<3 && $data['content_param']['merchant_list'][$i]['TypeID']!=1){ ?><?php if ($data['content_param']['merchant_list'][$i]['ID']==$data['content'][0]['MerchantID']) { ?> selected="selected"<?php }} else {?>disabled="disabled"<?php } ?>><?php //echo $data['content_param']['merchant_list'][$i]['ID']; ?><?php echo $data['content_param']['merchant_list'][$i]['Name']; ?> - <?php echo $data['content_param']['merchant_list'][$i]['TypeLabel']; ?></option>
          <?php } ?>
        </select></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><input name="DateExpiry" type="text" class="validate[required,custom[dmyDateTime]] datepicker mask_date" value="<?php echo $data['content'][0]['DateExpiry']; ?>" size="20" maxlength="20" /><span class="label_hint">(dd-mm-yyyy hh:mm:ss)</span></td>
    </tr>
    <!-- <tr>
