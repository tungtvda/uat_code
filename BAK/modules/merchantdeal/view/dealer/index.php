...
<div class="notify">Dealer Deal deleted successfully.</div>
<?php } ?>
<div id="search_box"<?php if ($data['content_param']['search']=="off") { ?>class="search_initial"<?php } ?>>
  <h2>Search</h2><span id="search_trigger_box">(<a id="search_trigger" href="javascript:void(0);">click to show/hide</a>)</span>
  <div id="search_content" <?php if ($data['content_param']['search']=="off") { ?>class="invisible"<?php } ?>>
	  <p>Submitting this search form without any data entry will show all results. Clicking on the Reset button will also remove all filters and show all results.</p>
	  <form name="search_form" class="admin_table_nocell"  id="search_form" action="<?php echo $data['config']['SITE_URL']; ?>/dealer/merchantdeal/index" method="post">
	    <input name="Trigger" type="hidden" value="search_form" />
	    <table border="0" cellspacing="0" cellpadding="0">
	      <tr>
	        <th scope="row"><label>Date Expiry (From)</label></th>
            <td><input name="DateExpiryFrom" class="datepicker" type="text" value="<?php echo $_SESSION['merchantdeal_DealerIndex']['param']['DateExpiryFrom']; ?>" size="20" />
	      ...
	      <tr>
            ...
            <td><input name="DateExpiryTo" class="datepicker" type="text" value="<?php echo $_SESSION['merchantdeal_DealerIndex']['param']['DateExpiryTo']; ?>" size="20" />
              (dd-mm-yyyy hh:mm:ss)</td>
	        <td>&nbsp;</td>
	        <td>Merchant</td>
	        <td><select name="MerchantID" class="chosen">
	            ...
	            <option value="<?php echo $data['content_param']['merchant_list'][$i]['ID']; ?>" <?php if ($data['content_param']['merchant_list'][$i]['ID']==$_SESSION['merchantdeal_DealerIndex']['param']['MerchantID']) { ?> selected="selected"<?php } ?>><?php echo $data['content_param']['merchant_list'][$i]['ID']; ?> - <?php echo $data['content_param']['merchant_list'][$i]['Name']; ?></option>
	            <?php } ?>
	          </select></td>
	      </tr>
	      <tr>
	      ...
	      <tr>
	        ...
	        <td class="text_right"><input type="submit" name="submit" value="Search" class="button" />
	          <a href="<?php echo $data['config']['SITE_URL']; ?>/dealer/merchantdeal/index?page=all">
...
<div class="admin_results">
  ...
  <div class="results_right"><a href='<?php echo $data['config']['SITE_URL']; ?>/dealer/merchantdeal/add/'>
    <input type="button" class="button" value="Create Dealer Deal">
    </a><?php /*?><a href='<?php echo $data['config']['SITE_URL']; ?>/dealer/merchantdeal/export/AdminIndex'>
...
<table class="member_table" border="0" cellspacing="0" cellpadding="0">
  ...
  <tbody <?php if ($data['content_param']['sort']==TRUE) { ?>class="sortable"<?php } ?>>
    ...
    <td class="center"><?php echo $data['content'][$i]['MerchantID']; ?></td>
    <td class="center"><?php echo $data['content'][$i]['DateExpiry']; ?></td>
    <td><div align="center"><a href='<?php echo $data['config']['SITE_URL']; ?>/dealer/merchantdeal/edit/<?php echo $data['content'][$i]['ID']; ?>'>Edit</a></div></td>
    <td><div align="center"><a href='<?php echo $data['config']['SITE_URL']; ?>/dealer/merchantdeal/delete/<?php echo $data['content'][$i]['ID']; ?>' onclick='return call_confirm()'>Delete</a></div></td>
  </tr>
  <?php } ?>
  </tbody>
</table>
