<?php if ($data['page']['admin_add']['error']['count']>0) { ?>
    <div class="error">
        <ul>
            <?php if ($data['page']['admin_add']['error']['Username']==1) { ?><li>The username "<?php echo $data['form_param']['Username']; ?>" is taken. Please try again with another username.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['NRIC']==1) { ?><li>The NRIC number <?php echo $data['form_param']['NRIC']; ?> exists in the database. Each agent can only register once. Please try again with another NRIC number.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['Passport']==1) { ?><li>The Passport number <?php echo $data['form_param']['Passport']; ?> exists in the database. Each agent can only register once. Please try again with another Passport number.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['Bank']==1) { ?><li>The Bank <?php echo $data['form_param']['Bank']; ?> exists in the database. Please try again with another bank.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['Email']==1) { ?><li>The Email: <?php echo $data['form_param']['Email']; ?> exists in the database. Please try again with another email.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail1']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail1']; ?> exists in the database. Please try again with another platform email 1.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail2']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail2']; ?> exists in the database. Please try again with another platform email 2.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail3']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail3']; ?> exists in the database. Please try again with another platform email 3.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail4']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail4']; ?> exists in the database. Please try again with another platform email 4.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail5']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail5']; ?> exists in the database. Please try again with another platform email 5.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail6']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail6']; ?> exists in the database. Please try again with another platform email 6.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail7']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail7']; ?> exists in the database. Please try again with another platform email 7.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail8']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail8']; ?> exists in the database. Please try again with another platform email 8.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail9']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail9']; ?> exists in the database. Please try again with another platform email 9.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['PlatformEmail10']==1) { ?><li>The Email: <?php echo $data['form_param']['PlatformEmail10']; ?> exists in the database. Please try again with another platform email 10.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['Company']==1) { ?><li>The Company URL: <?php echo $data['form_param']['Company']; ?> exists in the database. Please try again with another Company URL.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['Host']==1) { ?><li>The Host: <?php echo $data['form_param']['Host']; ?> was invalid.</li><?php } ?>
            <?php if ($data['page']['admin_add']['error']['DNS']==1) { ?><li>Failed to added DNS record, status message: <?php echo $data['page']['admin_add']['dnsMessage']['statusDescription']; ?>.</li><?php } ?>
        </ul>
    </div>
<?php } ?>
<form name="add_form" class="admin_table_noMobile" id="add_form" action="<?php echo $data['config']['SITE_DIR']; ?>/admin/agent/addprocess" enctype="multipart/form-data" method="post">
  <table border="0" Mobilespacing="0" Mobilepadding="0">
          ...
          <?php if($data['content_param']['product_list']['count']>'0'){ ?>
            ...
            <?php for ($i=0; $i<$data['content_param']['product_list']['count']; $i++) { ?>
              <?php if($i % 2==0){ ?>
                        ...
                        <tr>               
                    <?php } ?> 
              <td><input type="checkbox" name="Product[]" value="<?php echo $data['content_param']['product_list'][$i]['ID']; ?>">&nbsp;<?php echo $data['content_param']['product_list'][$i]['Name']; ?>&nbsp;&nbsp;</td>
                    <?php if($i % 2!==0 && ($i!==0)){ ?>
    ...
    <tr>
      ...
      <td><input name="Profitsharing" class="validate[required, custom[onlyNumberSp], min[0], max[100]]" type="text" value="<?php echo $data['form_param']['Profitsharing']; ?>" size="10" /><span class="label_hint">(min = 0, max = 100)</span></td>
    </tr>
    <tr>
      <th scope="row"><label>Parent</label></th>
      <td><select name="ParentID" class="validate[] chosen">                       
                        <option value="0"<?php if($data['form_param']['ParentID']=='0'){ ?> selected="selected" <?php } ?>>--None--</option> 
                        <?php for ($i=0; $i<$data['content_param']['agent_list']['count']; $i++) { ?>
                        <option value="<?php echo $data['content_param']['agent_list'][$i]['ID']; ?>" <?php if($data['form_param']['ParentID']==$data['content_param']['agent_list'][$i]['ID']){ ?> selected="selected" <?php } ?>><?php echo $data['content_param']['agent_list'][$i]['Name']; ?></option>
                        <?php } ?>
                  </select>
          </td>
    </tr>
    <tr>
      ...
      <td><select name="TypeID" class="validate[] chosen">
                        ...
                        <option value="<?php echo $data['content_param']['agenttype_list'][$i]['ID']; ?>" <?php if($data['form_param']['TypeID']==$data['content_param']['agenttype_list'][$i]['ID']){ ?> selected="selected" <?php } ?>><?php echo $data['content_param']['agenttype_list'][$i]['Label']; ?></option>
                        <?php } ?>
                  </select>
          </td>
    </tr> 
...
<!--<tr>
    ...
    <tr>
      ...
      <td><input name="Company" class="validate[required]" type="text" value="<?php echo $data['form_param']['Company']; ?>" size="40" /><span class="label_hint">(Please input only allowed formats, e.g. youtube.com, facebook.com. You shouldn't include www or http://www in front of domain)</span></td>
    </tr>   
    <tr>
...
<!--<tr>
    ...
    <tr>
      ...
      <td><input name="IPAddress" type="text" class="validate[required]" value="<?php echo $data['form_param']['IPAddress']; ?>" size="20" /></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><input name="Email" type="text" class="validate[required, custom[email]]" value="<?php echo $data['form_param']['Email']; ?>" size="30" /><span class="label_hint">(e.g. harry.porter@gmail.com) Note: If you are creating platform agent, please type your email.</span></td>
    </tr>
    <tr>
...
<!--    <tr>
    ...
    <!--<tr>
      ...
      <td><input name="Colour" type="text" class="validate[required] cpicker" value="<?php echo $data['form_param']['Colour']; ?>" size="30" /></td>
    </tr>-->
    <tr>
      <th scope="row">&nbsp;</th>
      <td>&nbsp;</td>
