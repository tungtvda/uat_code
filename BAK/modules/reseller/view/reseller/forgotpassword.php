<?php if ($data['page']['reseller_forgotpassword']['error']['count']>0) { ?>
    <?php if ($data['page']['reseller_forgotpassword']['error']['NoMatch']==1) { ?>
    <div class="error">The username and email do not match. Please try again.</div>
    <?php } ?>
<?php } ?>
<p>Please enter your username below and submit to retrieve a new password. If you have also lost your username or registered email, please <a href="<?php echo $data['config']['SITE_DIR']; ?>/contact#form">contact us</a> for further assistance.</p>
<form name="forgotpassword_form" class="admin_table_noMobile"  id="forgotpassword_form" action="<?php echo $data['config']['SITE_DIR']; ?>/reseller/reseller/forgotpasswordprocess" method="post">
