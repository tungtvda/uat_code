<?php if ($data['page']['reseller_add']['error']['count']>0) { ?>
    <div class="error">
        <ul>
            <?php if ($data['page']['reseller_add']['error']['Username']==1) { ?><li>The username "<?php echo $data['form_param']['Username']; ?>" is taken. Please try again with another username.</li><?php } ?>
            <?php if ($data['page']['reseller_add']['error']['NRIC']==1) { ?><li>The NRIC number <?php echo $data['form_param']['NRIC']; ?> exists in the database. Each reseller can only register once. Please try again with another NRIC number.</li><?php } ?>
            <?php if ($data['page']['reseller_add']['error']['Passport']==1) { ?><li>The Passport number <?php echo $data['form_param']['Passport']; ?> exists in the database. Each reseller can only register once. Please try again with another Passport number.</li><?php } ?>
            <?php if ($data['page']['reseller_add']['error']['Bank']==1) { ?><li>The Bank <?php echo $data['form_param']['Bank']; ?> exists in the database. Please try again with another bank.</li><?php } ?>
        </ul>
    </div>
<?php } ?>
<form name="add_form" class="admin_table_noMobile" id="add_form" action="<?php echo $data['config']['SITE_DIR']; ?>/admin/reseller/addprocess" method="post">
  <table border="0" Mobilespacing="0" Mobilepadding="0">
    ...
    <tr>
      ...
      <td><input name="Company" class="validate[required]" type="text" value="<?php echo $data['form_param']['Company']; ?>" size="40" /></td>
    </tr> 
    <!--<tr>
