<?php if ($data['page']['productcategory_add']['error']['count']>0) { ?>
    <?php if ($data['page']['productcategory_add']['error']['ImageURL']!="") { ?>
    <div class="error">Image upload failed (Error: <?php echo $data['page']['productcategory_add']['error']['ImageURL']; ?>)</div>
    <?php } ?>
<?php } else { ?>
    <?php if ($data['page']['productcategory_add']['ok']==1) { ?>
    <div class="notify">Product Category created successfully.</div>
    <?php } ?>
<?php } ?>
<?php if ($data['page']['productcategory_edit']['error']['count']>0) { ?>
    <?php if ($data['page']['productcategory_edit']['error']['ImageURL']!="") { ?>
    <div class="error">Image upload failed (Error: <?php echo $data['page']['productcategory_edit']['error']['ImageURL']; ?>)</div>
    <?php } ?>
<?php } else { ?>
    <?php if ($data['page']['productcategory_edit']['ok']==1) { ?>
    <div class="notify">Product Category edited successfully.</div>
    <?php } ?>
<?php } ?>
<form name="edit_form" class="admin_table_nocell"  id="edit_form" enctype="multipart/form-data" action="<?php echo $data['config']['SITE_DIR']; ?>/admin/productcategory/editprocess/<?php echo $data['content'][0]['ID']; ?>" method="post">
  <table border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td><select name="ParentID" class="chosen">
          ...
          <option value="<?php echo $data['content_param']['productcategory_list'][$i]['ID']; ?>" <?php if ($data['content_param']['productcategory_list'][$i]['ID']==$data['content'][0]['ParentID']) { ?> selected="selected"<?php } ?>><?php echo $data['content_param']['productcategory_list'][$i]['Name']; ?></option>
          <?php } ?>
        </select>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td><input name="Position" class="validate[required]" type="text" value="<?php echo $data['content'][0]['Position']; ?>" size="80" /></td>
    </tr>
    <tr>
      <th scope="row">&nbsp;</th>
      <td>&nbsp;</td>
    ...
    <tr>
      ...
      <td><input type="submit" name="submit" value="Update" class="button" />
        <a href="<?php echo $data['config']['SITE_DIR']; ?>/admin/productcategory/index">
