...
<form name="profileupgradeprocess_form" id="profileupgradeprocess_form_standard" action="<?php echo $data['config']['SITE_URL']; ?>/cart/order/confirm" method="post">
    ...
    <input type="hidden" name="OrderDesc" id="OrderDesc" value="Annual Membership Fee (RM<?php echo Helper::displayCurrency($data['config']['MEMBER_FEE']); ?>) (Period: <?php echo date('d-m-Y'); ?> to <?php echo date('d-m-Y',strtotime('+1 year')); ?>)" />
    <input type="hidden" name="OrderAmount" id="OrderAmount" value="<?php echo $data['config']['MEMBER_FEE']; ?>" />
