...
<form name="profileupgradeprocess_form" id="profileupgradeprocess_form_standard" action="<?php echo $data['config']['SITE_URL']; ?>/cart/order/confirm/<?php echo $_SESSION['DealerMerchant']['ID'] ?>" method="post">
    <input type="hidden" name="OrderType" id="OrderType" value="Standard Membership" />
    <input type="hidden" name="OrderItem" id="OrderItem" value="Standard Membership Upgrade" />
