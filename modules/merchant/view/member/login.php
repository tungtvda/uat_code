<div id="merchant_login_wrapper">
  <?php if ($data['page']['merchant_register']['ok']==1) { ?>
  <div class="notify">Registration is complete! You may now login below.</div>
  <?php } ?>
  <?php if ($data['page']['merchant_password']['ok']==1) { ?>
  <div class="notify">Your new password has been saved. Please login again to continue.</div>
  <?php } ?>
  <?php if ($data['page']['merchant_logout']['ok']==1) { ?>
  <div class="notify">You have logged out successfully.</div>
  <?php } ?>
  <?php if ($data['page']['merchant_forgotpassword']['ok']==1) { ?>
  <div class="notify">A new password has been generated and sent to your registered email.</div>
  <?php } ?>
  <?php if ($data['page']['merchant_login']['error']['count']>0) { ?>
    <?php if ($data['page']['merchant_login']['error']['Login']==1) { ?>
    <div class="error">The username and/or password entered is invalid. Please try again.</div>
    <?php } ?>
  <?php } ?>
  <form name="login_form" id="login_form" method="post" action="<?php echo $data['config']['SITE_URL']; ?>/merchant/merchant/loginprocess">
    <table border="0" cellspacing="0" cellpadding="0">
      ...
      <!-- <tr>
        <th scope="row">&nbsp;</th>
        <td class='subtext'><label><input type="checkbox" name="AutoLogin" value="1" <?php if (($data['page']['merchant_auth']==1)&&($_GET['autologin']==1)) { ?>checked="checked"<?php } ?> /> Keep me logged in</label></td>
      </tr> -->
      <tr>
        <th scope="row">&nbsp;</th>
        <td>&nbsp;</td>
      ...
      <tr>
        <th scope="row">&nbsp;</th>
        <td class='subtext'><div class="forgot_password"><a href="<?php echo $data['config']['SITE_URL']; ?>/merchant/merchant/forgotpassword">Forgot your password?</a></div></td>
      </tr>
    </table>
  </form>
</div>
