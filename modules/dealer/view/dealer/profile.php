<?php if ($data['page']['dealer_profile']['error']['count']>0) { ?>
    <div class="error">
        <ul>
            <?php if ($data['page']['dealer_profile']['error']['NRIC']==1) { ?><li>The NRIC number <?php echo $data['form_param']['NRIC']; ?> exists in our records. Each dealer can only register once. Please try again with another NRIC number.</li><?php } ?>
            <?php if ($data['page']['dealer_profile']['error']['Passport']==1) { ?><li>The Passport number <?php echo $data['form_param']['Passport']; ?> exists in our records. Each dealer can only register once. Please try again with another Passport number.</li><?php } ?>
        </ul>
    </div>
<?php } else { ?>
    <?php if ($data['page']['dealer_profile']['ok']==1) { ?>
    <div class="notify">Profile updated successfully.</div>
    <?php } ?>
<?php } ?>
<form name="profile_form" class="admin_table_nocell"  id="profile_form" action="<?php echo $data['config']['SITE_URL']; ?>/dealer/dealer/profileprocess/" method="post">
