<?php if ($data['page']['dealer_add']['error']['count']>0) { ?>
    <div class="error">
        <ul>
            <?php if ($data['page']['dealer_add']['error']['Username']==1) { ?><li>The username "<?php echo $data['form_param']['Username']; ?>" is taken. Please try again with another username.</li><?php } ?>
            <?php if ($data['page']['dealer_add']['error']['NRIC']==1) { ?><li>The NRIC number <?php echo $data['form_param']['NRIC']; ?> exists in the database. Each dealer can only register once. Please try again with another NRIC number.</li><?php } ?>
            <?php if ($data['page']['dealer_add']['error']['Passport']==1) { ?><li>The Passport number <?php echo $data['form_param']['Passport']; ?> exists in the database. Each dealer can only register once. Please try again with another Passport number.</li><?php } ?>
        </ul>
    </div>
<?php } ?>
<form name="add_form" class="admin_table_noMobile" id="add_form" action="<?php echo $data['config']['SITE_URL']; ?>/admin/dealer/addprocess" method="post">
