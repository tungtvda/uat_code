...
class TransactionModel extends BaseModel
{
    ...
    public function AdminIndex($param)
    {
        ...
        foreach ($this->dbconnect->query($sql) as $row) {
            ...
            if ($row['StaffID'] != 0) {
                $ModifiedPerson = $row['StaffID'];
                $ModifiedPerson = StaffModel::getStaff($row['StaffID'], "Username") . " (staff)";
            } else if ($row['OperatorID'] != 0) {
                $ModifiedPerson = $row['OperatorID'];
                $ModifiedPerson = OperatorModel::getOperator($row['OperatorID'], "Username") . " (agent)";
            }

            // Determine last update person
            if ($row['StaffIDUpdated'] != 0) {
                $UpdatedPerson = $row['StaffIDUpdated'];
                $UpdatedPerson = StaffModel::getStaff($row['StaffIDUpdated'], "Username") . " (staff)";
            } else if ($row['OperatorIDUpdated'] != 0) {
                $UpdatedPerson = $row['OperatorIDUpdated'];
                $UpdatedPerson = OperatorModel::getOperator($row['OperatorIDUpdated'], "Username") . " (agent)";
            }
            ...
            $result[$i] = array(
                ...
                'WalletID' => WalletModel::getWalletID($row['MemberID'], $row['ProductID']),
        ...
        $this->output = array(
            ...
            'content_param' => array('count' => $i, 'total_results' => $total_pages, 'paginate' => $paginate, 'query_title' => $query_title, 'search' => $search, 'enabled_list' => CRUD::getActiveList(), /*'member_list' => MemberModel::getMemberList(),*/
                'transactiontype_list' => TransactionTypeModel::getTransactionTypeList(), 'transactionstatus_list' => TransactionStatusModel::getTransactionStatusList(), 'rejecte_message' => RejectMessageModel::getRejectMessageList(), 'bank_list' => BankModel::getBankList(), 'agentpromotion_list' => AgentPromotionModel::getEnabledAgentPromotionList()),
            'secure' => TRUE,
            'meta' => array('active' => "on"));
    ...
    public function APITransferProcess()
    {
        ...
        if ($method == "POST") {
            ...
            if ($authenticate == "OK") {
                ...
                if ($request_data['Amount'] < '10') {
                    $restapi->setResponse('422', 'Amount must be RM10 or above.');
                }

                // Disallow product to product
                if (($request_data['TransferFrom'] != '30') && ($request_data['TransferTo'] != '30')) {
                    $restapi->setResponse('422', 'Invalid parameters.');
                }
                ...
                if (($request_data['TransferFrom'] == '') || ($request_data['TransferTo'] == '')) {
                    $restapi->setResponse('422', 'Invalid parameters.');
                }
    ...
    public function AdminEditProcess($param)
    {
        ...
        if ($ModifierDetails['ModifiedDate'] == '0000-00-00 00:00:00') // If not updated before
        {
    ...
    public function AgentEditProcess($param)
    {
        ...
        if ($ModifierDetails['ModifiedDate'] == '0000-00-00 00:00:00') // If not updated before
        {
            ...
            $OperatorID = $_SESSION['agent']['operator']['RowID'];
            $OperatorIDUpdated = $_SESSION['agent']['operator']['RowID'];
        ...
        $sql = "UPDATE transaction SET
                        TypeID='" . $_POST['TypeID'] . "', MemberID='" . $_POST['MemberID'] . "', Description='" . $_POST['Description'] . "', Promotion='" . $_POST['PromoSpecial'] . "', ProductID='" . $_POST['ProductID'] . "', BankSlip='" . $_POST['BankSlip'] . "', RejectedRemark='" . $_POST['RejectedRemark'] . "', Date='" . Helper::dateTimeDisplaySQL($_POST['Date']) . "', TransferTo='" . $_POST['TransferTo'] . "', TransferFrom='" . $_POST['TransferFrom'] . "', Debit='" . $_POST['Debit'] . "', Credit='" . $_POST['Credit'] . "', Amount='" . $_POST['Amount'] . "', DepositBonus='" . $_POST['DepositBonus'] . "', Bonus='" . $_POST['Bonus'] . "', Commission='" . $_POST['Commission'] . "', ReferenceCode='" . $_POST['ReferenceCode'] . "', Bank='" . $_POST['Bank'] . "', Status='" . $_POST['Status'] . "', StaffID = '" . $StaffID . "', StaffIDUpdated = '" . $StaffIDUpdated . "', OperatorID = '" . $OperatorID . "', OperatorIDUpdated = '" . $OperatorIDUpdated . "', ModifiedDate = '" . $ModifiedDate . "', UpdatedDate = '" . $UpdatedDate . "' WHERE ID='" . $param . "'";
    ...
    public function AdminQuickEditProcess($param)
    {
        ...
        $sql = "UPDATE transaction SET RejectedRemark='" . $_POST['RejectedRemark'] . "', Bonus='" . $_POST['Bonus'] . "', Commission='" . $_POST['Commission'] . "', Status='" . $_POST['Status'] . "', StaffID = '" . $StaffID . "', StaffIDUpdated = '" . $StaffIDUpdated . "', OperatorID = '" . $OperatorID . "', OperatorIDUpdated = '" . $OperatorIDUpdated . "', ModifiedDate = '" . $ModifiedDate . "', UpdatedDate = '" . $UpdatedDate . "' WHERE ID='" . $param . "'";
        /*echo $sql;
            exit;*/
        $count = $this->dbconnect->exec($sql);
    ...
    public function AgentQuickEditProcess($param)
    {
       ...
       else
       {
        ...
        $sql = "UPDATE transaction SET RejectedRemark='" . $_POST['RejectedRemark'] . "', Bonus='" . $_POST['Bonus'] . "', Commission='" . $_POST['Commission'] . "', StaffID = '" . $StaffID . "', StaffIDUpdated = '" . $StaffIDUpdated . "', OperatorID = '" . $OperatorID . "', OperatorIDUpdated = '" . $OperatorIDUpdated . "', ModifiedDate = '" . $ModifiedDate . "', UpdatedDate = '" . $UpdatedDate . "', Status='" . $_POST['Status'] . "' WHERE ID='" . $param . "'";
        /*echo $sql;
            exit;*/
        $count = $this->dbconnect->exec($sql);
    ...
    public function AdminAffiliatedReporting($param)
    {
        ...
        if (empty($param) === TRUE) {
            ...
            #Debug::displayArray($currentAgent);
        ...
        $this->output = array(
            'config' => $this->config,
            'page' => array('title' => "View Affiliate Report", 'template' => 'admin.common.tpl.php', 'custom_inc' => 'on', 'custom_inc_loc' => $this->module_dir . 'inc/admin/affiliatedreporting.inc.php'),
...
//        if (in_array($_POST['form_token'], $_SESSION['member']['token']) === FALSE) {
        if (in_array($_POST['form_token'], $_SESSION['member']['token']) === FALSE) {
            ...
            $date_deposit = "Date of Deposit: " . $this->checkPostParamSecurity('DateDeposit');
            $date_posted = date('YmdHis');
            $date_posted_display = date('d-m-Y H:i:s');
            // get param

            $PromoSpecial=$this->checkPostParamSecurity('PromoSpecial');
            ...
            if($DepositChannel=='voucher'){
                ...
                $password_code=$this->checkPostParamSecurity('password_code');
                $check=$this->membercheckCodeArray($code_card, $password_code);
                if($check==0){
                    return 0;
                }
                else{
    ...
    public function membercheckCode($code, $pass)
    {
        $pass=trim($pass);
        $code=trim($code);
        $dk = 'Password="' . $pass . '" and Status=0';
        $query_count = "SELECT COUNT(*) AS num FROM agent_voucher_password WHERE " . $dk;
        $total_pages = $this->dbconnect->query($query_count)->fetchColumn();
        if ($total_pages > 0) {
            $sql = "SELECT * FROM agent_voucher WHERE Code='" . $code . "'";
            foreach ($this->dbconnect->query($sql) as $row) {
                return $row['Amount'];
            }
                return 0.0000006;
        } else {
            $sql_pass = "SELECT COUNT(*) AS num FROM agent_voucher WHERE Code='" . $code . "'";
            $total_pages_pass = $this->dbconnect->query($sql_pass)->fetchColumn();
            if($total_pages_pass==0)
            {
                return 0.00000007;
            }
            else{
    ...
    public function membercheckCodeArray($code, $pass)
    {
        $pass=trim(addslashes(strip_tags($pass)));
        $code=trim(addslashes(strip_tags($code)));
        $dk = 'Password="' . $pass . '" and Status=0';
        $sql_pass = "SELECT * FROM agent_voucher_password WHERE " . $dk;
