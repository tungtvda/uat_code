...
class POI extends BaseController
{
	...
	protected function AddProcess()
	{
		...
		$_SESSION['admin']['poi_add'] = $param['status'];

        if ($param['status']['ok']=="1")
		{
			Helper::redirect($param['config']['SITE_URL']."/admin/poi/edit/".$param['content_param']['newID']);
		}
		else
		{
			Helper::redirect($param['config']['SITE_URL']."/admin/poi/add/");
		}
		
		
	}
	
	protected function SearchProcess()
	{
	...
	protected function Edit()
	{
        ...
        {
            Helper::redirect($param['config']['SITE_URL']."/admin/poi/index");
        }
	}
	...
	protected function EditProcess()
	{
		...
		$_SESSION['admin']['poi_edit'] = $param['status'];

        if ($param['status']['ok']=="1")
		{
			Helper::redirect($param['config']['SITE_URL']."/admin/poi/edit/".$this->id);
		}
		else
		{
			Helper::redirect($param['config']['SITE_URL']."/admin/poi/edit/".$this->id);
	...
	protected function Delete()
	{
		...
		$_SESSION['admin']['poi_delete'] = $param['status'];
