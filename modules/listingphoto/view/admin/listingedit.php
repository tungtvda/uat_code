<?php if ($data['page']['listingphoto_listingadd']['error']['count']>0) { ?>
    <?php if ($data['page']['listingphoto_listingadd']['error']['ImageURL']!="") { ?>
    <div class="error">Image upload failed (Error: <?php echo $data['page']['listingphoto_listingadd']['error']['ImageURL']; ?>)</div>
    <?php } ?>
<?php } else { ?>
    <?php if ($data['page']['listingphoto_listingadd']['ok']==1) { ?>
    <div class="notify">Listing Photo created successfully.</div>
    <?php } ?>
<?php } ?>
<?php if ($data['page']['listingphoto_listingedit']['error']['count']>0) { ?>
    <?php if ($data['page']['listingphoto_listingedit']['error']['ImageURL']!="") { ?>
    <div class="error">Image upload failed (Error: <?php echo $data['page']['listingphoto_listingedit']['error']['ImageURL']; ?>)</div>
    <?php } ?>
<?php } else { ?>
    <?php if ($data['page']['listingphoto_listingedit']['ok']==1) { ?>
    <div class="notify">Listing Photo edited successfully.</div>
    <?php } ?>
<?php } ?>
<form action="<?php echo $data['config']['SITE_URL']; ?>/admin/listingphoto/listingeditprocess/<?php echo $data['parent']['id']; ?>,<?php echo $data['content'][0]['ID']; ?>" method="post" enctype="multipart/form-data" name="edit_form" class="admin_table_nocell"  id="edit_form">
  <table border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      ...
      <td><input type="submit" name="submit" value="Update" class="button" />
        <a href="<?php echo $data['config']['SITE_URL']; ?>/admin/listingphoto/listingindex/<?php echo $data['parent']['id']; ?>">
