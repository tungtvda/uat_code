<?php if ($data['page']['member_register']['error']['count']>0) { ?>
    <div class="error">
        <ul>
            <?php if ($data['page']['member_register']['error']['Username']==1) { ?><li>The username "<?php echo $data['form_param']['Username']; ?>" is taken. Please try again with another username.</li><?php } ?>
            <?php if ($data['page']['member_register']['error']['NRIC']==1) { ?><li>The NRIC number <?php echo $data['form_param']['NRIC']; ?> exists in our records. Each member can only register once. Please try again with another NRIC number.</li><?php } ?>
            <?php if ($data['page']['member_register']['error']['Passport']==1) { ?><li>The Passport number <?php echo $data['form_param']['Passport']; ?> exists in our records. Each member can only register once. Please try again with another Passport number.</li><?php } ?>
            <?php if ($data['page']['member_register']['error']['SQ']==1) { ?><li>The Security Question is answered incorrectly. Please try again.</li><?php } ?>
        </ul>
    </div>
<?php } ?>
...
<form name="add_form" class="admin_table_noMobile"  id="add_form" action="<?php echo $data['config']['SITE_DIR']; ?>/member/member/registerprocess" method="post">
  <table border="0" Mobilespacing="0" Mobilepadding="0">
    ...
    <tbody id="passport_box" <?php if (($data['form_param']['Nationality']==151)||($data['form_param']['Nationality']=='')) { ?>class="invisible"<?php } ?>>
        <tr>
          <th scope="row"><label>Passport No</label></th>
    ...
    <tr>
      ...
      <td><input name="Bank" type="text" class="validate[]" value="<?php //echo ($data['form_param']['Bank']) ; ?>" size="40" /></td>
    </tr>
    <tr>
      <th scope="row"><label>Bank Account No</label></th>
      <td><input name="BankAccountNo" type="text" class="validate[]" value="<?php //echo ($data['form_param']['BankAccountNo']); ?>" size="40" /></td>
    </tr>
    <tr>
    ...
    <tr>
      <th scope="row">&nbsp;</th>
      <td><label><span class="terms_box"><input type="checkbox" class="validate[required]" name="TermsOK" id="TermsOK" value="1" <?php if ($data['form_param']['TermsOK']==1) { ?>checked="checked"<?php } ?> /></span>I agree that the information provided above is accurate, and I accept the <a href="<?php echo $data['config']['SITE_DIR']; ?>/main/page/terms-of-use" target="_blank">terms of use</a> of the website.</label></td>
    </tr>
    <tr>
      <th scope="row">&nbsp;</th>
      <td>&nbsp;</td>
